...
- Throughout the repository, the name `humanoid_lab` only serves as an example and we provide a script to rename all the references to it automatically:

```bash
# Enter the repository
cd IsaacLabExtensionTemplate
# Rename all occurrences of humanoid_lab (in files/directories) to your_fancy_extension_name
...
```bash
python -m pip install -e exts/humanoid_lab
```
...
```json
{
    "python.analysis.extraPaths": [
        "<path-to-ext-repo>/exts/humanoid_lab"
    ]
}
```
